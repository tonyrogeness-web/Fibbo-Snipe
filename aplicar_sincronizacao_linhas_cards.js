...
const targetBlockStart = '   int cw_fr = show_fibo_card ? ((pw - (pad * 2) - 4) / 2) : (pw - (pad * 2));';
const targetBlockEnd = '   cur+=ch+22;';

const startIdx = content.indexOf(targetBlockStart);
const endIdx = content.indexOf(targetBlockEnd, startIdx);

if (startIdx !== -1 && endIdx !== -1) {
  const replacement = `   // --- VALIDAÇÃO DE LINHA CONTÍNUA E ARMADO REAL DO FR ---
  ...
  console.log('✅ Bloco dos cards atualizado com sincronização perfeita de linhas contínuas vs pontilhadas!');
} else {
  console.log('⚠️ Índices não encontrados!');
}
