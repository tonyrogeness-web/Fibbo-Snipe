...
console.log('=== ATUALIZANDO DEPLOY EX5 E PRESET FR+FLUXO COM ROTEAMENTO ===\n');

// 1. Deploy do .EX5 para todas as pastas de Experts do MT5
const ex5Source = path.join(__dirname, 'Fibbo_Sniper_v28.5_H2.ex5');
const expertDirs = [
...
expertDirs.forEach(dir => {
  if (fs.existsSync(dir)) {
    fs.copyFileSync(ex5Source, path.join(dir, 'Fibbo_Sniper_v28.5_H2.ex5'));
    console.log(`✔ Binário .EX5 copiado com sucesso para: ${dir}`);
  }
});

// 2. Criação do Preset Atualizado Fibbo_Sniper_v28.5_H2_FR+Fluxo.set
function buildSetLines(overrides = {}) {
  const defaults = {
    ...
    "InpFR_BlockedSymbols": "GBPJPY,GBPUSD",
