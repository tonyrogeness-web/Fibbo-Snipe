...
console.log('=== ATUALIZANDO E VALIDANDO DEFINITIVAMENTE TODOS OS ARQUIVOS .SET ===\n');
...
setPaths.forEach(p => {
  try {
    ...
    console.log('✔ .SET regravado e atualizado em:', p);
  } catch (err) {
    console.log('Erro ao salvar .SET em:', p, err.message);
  }
});

// Sincroniza também o MQ5 mestre nas duas pastas de experts
...
expertPaths.forEach(p => {
  try {
    ...
    console.log('Erro ao salvar .MQ5 em:', p, err.message);
  }
});

console.log('\n=== TODOS OS PARÂMETROS E CÓDIGOS ESTÃO 100% REGRAVADOS E SINCRONIZADOS! ===');
