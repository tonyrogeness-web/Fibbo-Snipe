...
InpFiboBlockedSymbols=EURGBP,EURAUD||0||0||0||N
...
InpFib_MaxPenetrationATR=0.75||0.0||0.0||0.0||N
InpFib_RequireVolumeAbsorption=true||0||0||0||N
InpFib_MinVolumeRatio=0.90||0.0||0.0||0.0||N
...
const KEEP = 'Fibbo_Sniper_v28.5_H2.set';

targetDirs.forEach(dir => {
  if (fs.existsSync(dir)) {
    // Escreve o único arquivo oficial atualizado
    const p = path.join(dir, KEEP);
    fs.writeFileSync(p, buffer);
    console.log('✅ Arquivo Único Atualizado com FR + Fibo 2.0 no H2:', p);

    // Remove qualquer outro .set para não poluir
    fs.readdirSync(dir).forEach(f => {
      if (f.endsWith('.set') && f !== KEEP) {
