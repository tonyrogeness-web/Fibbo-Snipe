...
console.log('🔄 Salvando a configuração oficial da OPÇÃO C (PORTFÓLIO MESTRE COMPLETO)...');
...
// Configuração Opção C Mestre (InpAutoTF=true, InpBaseRisk_L1=2.0, InpTP_Final_Multi=3.5)
const opcaoCContent = `InpPerfil=1||0||0||2||N
...
writeUtf16LE(path.join(testerDir, 'Opcao_C_Portfolio_Mestre.set'), opcaoCContent);
writeUtf16LE(path.join(localDir, 'Opcao_C_Portfolio_Mestre.set'), opcaoCContent);
writeUtf16LE(path.join(testerDir, 'teste.set'), opcaoCContent);

console.log('\n🎉 OPÇÃO C (PORTFÓLIO MESTRE COMPLETO) COMPILADA E SALVA COM SUCESSO!');
