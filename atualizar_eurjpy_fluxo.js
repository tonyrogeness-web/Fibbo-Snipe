...
console.log('=== MOVENDO EURJPY PARA FLUXO INSTITUCIONAL E ATUALIZANDO .SET ===\n');

// 1. Atualizar Fibbo_Sniper_v28.5_H2.mq5
const mq5Path = path.join(__dirname, 'Fibbo_Sniper_v28.5_H2.mq5');
let code = fs.readFileSync(mq5Path, 'utf8');

const oldFRBlocked = 'input string InpFR_BlockedSymbols    = "GBPJPY,GBPUSD,XAUUSD,GOLD";';
const newFRBlocked = 'input string InpFR_BlockedSymbols    = "GBPJPY,GBPUSD,XAUUSD,GOLD,EURJPY"; // Pares bloqueados para FR (Tendência/Ouro/Iene)';

const oldFluxoBlocked = 'input string InpFluxo_BlockedSymbols = "EURUSD,EURJPY,EURCAD,NZDUSD,EURAUD,AUDNZD";';
const newFluxoBlocked = 'input string InpFluxo_BlockedSymbols = "EURUSD,EURCAD,NZDUSD,EURAUD,AUDNZD"; // Pares bloqueados para Fluxo (Pares de Canal/Range)';

code = code.replace(oldFRBlocked, newFRBlocked);
code = code.replace(oldFluxoBlocked, newFluxoBlocked);

fs.writeFileSync(mq5Path, code, 'utf8');
console.log('✔ Inputs padrão de InpFR_BlockedSymbols e InpFluxo_BlockedSymbols atualizados no .mq5!');

// 2. Atualizar geradores de .set
function buildSetLines(overrides = {}) {
  const defaults = {
    ...
    "InpFR_BlockedSymbols": "GBPJPY,GBPUSD,XAUUSD,GOLD,EURJPY",
    "InpFluxo_BlockedSymbols": "EURUSD,EURCAD,NZDUSD,EURAUD,AUDNZD",
    "InpAutoTF": "true",
...
console.log('\n=== ROTEAMENTO E .SET ATUALIZADOS COM EURJPY NO FLUXO! ===');
