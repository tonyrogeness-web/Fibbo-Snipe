...
console.log('=== ADICIONANDO XAUUSD / GOLD AO FLUXO E BLOQUEANDO NO FR ===\n');

// 1. Atualizar no código Fibbo_Sniper_v28.5_H2.mq5
const mq5Path = path.join(__dirname, 'Fibbo_Sniper_v28.5_H2.mq5');
let code = fs.readFileSync(mq5Path, 'utf8');

code = code.replace(
  'input string InpFR_BlockedSymbols    = "GBPJPY,GBPUSD";',
  'input string InpFR_BlockedSymbols    = "GBPJPY,GBPUSD,XAUUSD,GOLD"; // Pares bloqueados para FR (Tendência/Ouro)'
);

fs.writeFileSync(mq5Path, code, 'utf8');
console.log('✔ InpFR_BlockedSymbols atualizado no .mq5 com XAUUSD,GOLD.');

// 2. Atualizar nos arquivos .set
