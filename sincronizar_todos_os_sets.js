...
console.log('=== SINCRONIZANDO E ATUALIZANDO TODOS OS ARQUIVOS .SET ===\n');

// 1. Array base de entradas atualizadas
function buildSetLines(overrides = {}) {
  const defaults = {
    ...
    "InpUseFiboPullback": "true",
    "InpSmartFiboSymbolFilter": "true",
...
// 2. Definir os 3 presets principais
const presets = {
  // 1. Oficial: FR + Fibo
  "Fibbo_Sniper_v28.5_H2.set": buildSetLines({
    "InpUseFR": "true",
    "InpUseFiboPullback": "true",
    "InpUseFluxo": "false",
    "InpBaseRisk_L1": "1.5",
  ...
  // 2. FR + Fluxo Rigoroso
  "Fibbo_Sniper_v28.5_H2_fr+fluxo.set": buildSetLines({
    "InpUseFR": "true",
  ...
  // 3. Apenas Fluxo (Isolado para pares de tendência)
...
// 3. Pastas de Destino nos Terminais MT5
const terminalDirs = [
...
for (const [filename, content] of Object.entries(presets)) {
  // Salvar no workspace local
  fs.writeFileSync(path.join(__dirname, filename), content, 'utf16le');
  console.log(`✔ Salvo no Workspace: ${filename}`);

  // Salvar em cada pasta MT5
  terminalDirs.forEach(dir => {
    if (fs.existsSync(dir)) {
      fs.writeFileSync(path.join(dir, filename), content, 'utf16le');
      console.log(`  ➔ Copiado para MT5: ${path.join(dir, filename)}`);
    }
  });
}

console.log('\n=== TODOS OS .SETS ATUALIZADOS E SINCRONIZADOS COM SUCESSO! ===');
