...
console.log('=== GERANDO ARQUIVOS .SET INDIVIDUAIS (SOMENTE FR E SOMENTE FLUXO) ===\n');

function buildSetContent(overrides = {}) {
  const base = {
    "InpPerfil": "1",
...
// 1. SET SOMENTE FR (Falso Rompimento Puro)
const setFR = buildSetContent({
  "InpSmartRouting": "false",
  "InpUseFR": "true",
  "InpUseFluxo": "false"
});

// 2. SET SOMENTE FLUXO (Fluxo Institucional Puro)
const setFluxo = buildSetContent({
  "InpSmartRouting": "false",
  "InpUseFR": "false",
  "InpUseFluxo": "true"
});

// 3. SET UNIFICADO INTELIGENTE
const setUnificado = buildSetContent({
...
const sets = [
  { name: "Fibbo_Sniper_v28.5_H2_SOMENTE_FR.set", content: setFR },
  { name: "Fibbo_Sniper_v28.5_H2_SOMENTE_FLUXO.set", content: setFluxo },
  { name: "Fibbo_Sniper_v28.5_H2_FR+Fluxo.set", content: setUnificado }
];
...
sets.forEach(s => {
  targetDirs.forEach(dir => {
    if (fs.existsSync(dir)) {
      fs.writeFileSync(path.join(dir, s.name), s.content, 'utf16le');
      console.log(`✔ Salvo: ${s.name} em ${dir}`);
    }
  });
});

console.log('\n=== TODOS OS ARQUIVOS .SET INDIVIDUAIS FORAM CRIADOS E SALVOS COM SUCESSO! ===');
